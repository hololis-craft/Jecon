...
public class Create {
    ...
    private int executeWithBalance(CommandContext<CommandSourceStack> ctx) {
        return doCreate(ctx, BigDecimal.valueOf(DoubleArgumentType.getDouble(ctx, "balance")));
    }

    private int doCreate(CommandContext<CommandSourceStack> ctx, BigDecimal balance) {
        ...
        if (repository.hasAccount(target.uuid())) {
            sender.sendMessage(message.createAlready.toComponent("name", target.name()));
            return Command.SINGLE_SUCCESS;
        }
        repository.createAccount(target.uuid(), balance);
        sender.sendMessage(message.create.toComponent(
